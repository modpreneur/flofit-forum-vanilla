         ...
         <tbody>
            <tr>
               ...
               if ($this->User->Photo != '' && $AllowImages && !$RemotePhoto) {
                  echo Wrap(Anchor(T('Remove Picture'), CombinePaths(array(UserUrl($this->User, '', 'removepicture'), $Session->TransientKey())), 'Button ore Danger PopConfirm'), 'p');
               ?>
               </td>
               <td><?php
               echo $Thumbnail;
               echo Wrap(Anchor(T('Edit Thumbnail'), UserUrl($this->User, '', 'thumbnail'), 'Button ore'), 'p');
               }
               ?>
               </td>
            </tr>
