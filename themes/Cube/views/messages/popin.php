...
?>
   ...
   if ($Row['Subject']) {
      ...
      if($countOfUsers > 2)
      {
         $userPhotoElement = groupCustomProfilePhotoElement();
      }
      else
      {
         foreach ($Row['Participants'] as $User) {
            ...
            {
               $userPhotoElement = userCustomProfilePhotoElement($User["Name"]);
            }
         }
      }
   }
   ...
   <li class="Item" rel="<?php echo Url("/messages/{$Row['ConversationID']}#latest"); ?>">
      <div class="Author Photo"><?php echo $userPhotoElement; ?></div>
